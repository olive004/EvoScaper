...
def setup_model(fake_circuits_reshaped, config_bio: dict, input_species: List[str]):
    ...
    qreactions.init_properties(model_brn, config_bio['molecular_params'].get(['starting_concentration']))
...
def prep_sim(signal_species, qreactions, fake_circuits_reshaped, config_bio):
    ...
    def make_flat_triangle(matrices): return np.array(
        list(map(lambda i: i[np.triu_indices(n=matrices.shape[-1])], matrices)))
    ...
    t0, t1, dt0, dt1 = config_bio['simulation']['t0'], config_bio['simulation'][
        't1'], config_bio['simulation']['dt0'], config_bio['simulation']['dt1']
    save_steps, max_steps = 50, 16**5

    return signal_onehot, signal_target, y00, t0, t1, dt0, dt1, save_steps, max_steps
...
def verify(params, rng, decoder,
    ...
    fake_circuits_reshaped = np.array(list(map(partial(make_symmetrical_matrix_from_sequence_nojax,
                                      side_length=len(input_species)), fake_circuits.reshape(-1, fake_circuits.shape[-1]))))
    ...
    signal_onehot, signal_target, y00, t0, t1, dt0, dt1, save_steps, max_steps = prep_sim(
        signal_species, qreactions, fake_circuits_reshaped, config_bio)
